...
def run_single_scenario(scenario):
    # scenario is (density_label, stype, sname, args)
...
if __name__ == "__main__":
    ...
    # Generate 10 ring and 10 sbm for each density
    ...
    # Label them as SPARSE, INTERMEDIATE, DENSE
    # We'll flatten them all into one big list of (density_label, stype, sname, args).
    full_scenarios = []
    for scn in sparse_ring:
        stype, sname, args = scn
        full_scenarios.append(("SPARSE", stype, sname, args))
    for scn in sparse_sbm:
        stype, sname, args = scn
        full_scenarios.append(("SPARSE", stype, sname, args))

    for scn in interm_ring:
        stype, sname, args = scn
        full_scenarios.append(("INTERMEDIATE", stype, sname, args))
    for scn in interm_sbm:
        stype, sname, args = scn
        full_scenarios.append(("INTERMEDIATE", stype, sname, args))

    for scn in dense_ring:
        stype, sname, args = scn
        full_scenarios.append(("DENSE", stype, sname, args))
    for scn in dense_sbm:
        stype, sname, args = scn
        full_scenarios.append(("DENSE", stype, sname, args))

    print(f"Total scenarios: {len(full_scenarios)}")
    ...
    master_results = []
    with open("benchmark_log.csv", "w", newline="", buffering=1) as csvfile:
        w = csv.DictWriter(csvfile, fieldnames=fnames)
        w.writeheader()

        # Single parallel execution for *all* scenarios:
        with ProcessPoolExecutor(max_workers=50) as executor:
            ...
            for f in as_completed(future_to_scenario):
                ...
                print(f"[{idx}/{len(full_scenarios)}] {row['ScenarioName']} - {row['ScenarioDensity']} done")
                idx += 1
