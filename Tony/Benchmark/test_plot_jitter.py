...
for label in ["SPARSE", "INTERMEDIATE", "DENSE"]:
    ...
    x_ring = []
    # Time arrays
    t_ring_gn, t_ring_lp, t_ring_alp, t_ring_lv, t_ring_bvns, t_ring_gnx, t_ring_lnx = [], [], [], [], [], [], []
    # Memory arrays
    m_ring_gn, m_ring_lp, m_ring_alp, m_ring_lv, m_ring_bvns, m_ring_gnx, m_ring_lnx = [], [], [], [], [], [], []
    # Modularity arrays
    mod_ring_gn, mod_ring_lp, mod_ring_alp, mod_ring_lv, mod_ring_bvns, mod_ring_gnx, mod_ring_lnx = [], [], [], [], [], [], []
    # Conductance arrays
    cond_ring_gn, cond_ring_lp, cond_ring_alp, cond_ring_lv, cond_ring_bvns, cond_ring_gnx, cond_ring_lnx = [], [], [], [], [], [], []
    # ARI arrays
    ari_ring_gn, ari_ring_lp, ari_ring_alp, ari_ring_lv, ari_ring_bvns, ari_ring_gnx, ari_ring_lnx = [], [], [], [], [], [], []

    # Arrays for SBM
    x_sbm = []
    # Time arrays
    t_sbm_gn, t_sbm_lp, t_sbm_alp, t_sbm_lv, t_sbm_bvns, t_sbm_gnx, t_sbm_lnx = [], [], [], [], [], [], []
    # Memory arrays
    m_sbm_gn, m_sbm_lp, m_sbm_alp, m_sbm_lv, m_sbm_bvns, m_sbm_gnx, m_sbm_lnx = [], [], [], [], [], [], []
    # Modularity arrays
    mod_sbm_gn, mod_sbm_lp, mod_sbm_alp, mod_sbm_lv, mod_sbm_bvns, mod_sbm_gnx, mod_sbm_lnx = [], [], [], [], [], [], []
    # Conductance arrays
    cond_sbm_gn, cond_sbm_lp, cond_sbm_alp, cond_sbm_lv, cond_sbm_bvns, cond_sbm_gnx, cond_sbm_lnx = [], [], [], [], [], [], []
    # ARI arrays
    ari_sbm_gn, ari_sbm_lp, ari_sbm_alp, ari_sbm_lv, ari_sbm_bvns, ari_sbm_gnx, ari_sbm_lnx = [], [], [], [], [], [], []
    ...
    set_dynamic_ylim(ax_ari_r, ari_ring_gn + ari_ring_lp + ari_ring_alp + ari_ring_lv + ari_ring_bvns + ari_ring_gnx + ari_ring_lnx)

    fig_ring.tight_layout()
    fig_ring.savefig(f"z_{label.lower()}_ring.png")
    plt.close(fig_ring)
    ...
    set_dynamic_ylim(ax_ari_s, ari_sbm_gn + ari_sbm_lp + ari_sbm_alp + ari_sbm_lv + ari_sbm_bvns + ari_sbm_gnx + ari_sbm_lnx)

    fig_sbm.tight_layout()
    fig_sbm.savefig(f"z_{label.lower()}_sbm.png")
    plt.close(fig_sbm)
