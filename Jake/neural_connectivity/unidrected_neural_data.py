# Getting the 2024 Emmons data into an undirected graph
...
def main():
    input_data = pd.read_csv('symmetric_combined.csv', index_col=0) # Read the csv
    ...
    communities = bvns.bvns_with_communities_specified(G, 1, stop_criterion=2)
    utils.plot_graph_with_communities(G, communities)

def get_neuronal_connectivity_graph(file_path_from_caller: str = 'symmetric_combined.csv') -> nx.Graph:
